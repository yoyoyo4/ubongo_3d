...
end_flg = False # 実行を中断するための変数。ここで宣言しておいて関数内でglobal指定する
...
class DancingLinks:
    ...
    def algorithm_x(self, return_all_ans=True):
        """solves exact cover problem

        algorithm_x() -> List[List[int]]
        Exact Coverの部分集合のインデックス列を返す。
        return_all_ansがTrueのとき全てのExact Coverの部分集合を返し、Falseのとき1つだけ返す
        """
        result = []
        global end_flg # globalを使わないと関数内で変数を操作できない
        end_flg = False

        def _algorithm_x(sss, return_all_ans=True):
            global end_flg 
            if end_flg:
                return
            elif self.is_empty():
                result.append(set(sss))
                if not return_all_ans: # 解が1つ見つかれば次のループで即終了させる
                    end_flg = True
            else:
                # 列の選択
                ...
                while column_node is not min_header:
                    ...
                    _algorithm_x(sss, return_all_ans)
                    sss.pop()
                    self.uncover(column_node)
                    column_node = column_node.down

        _algorithm_x([], return_all_ans)
        return result
